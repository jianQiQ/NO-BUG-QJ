    ...
    while(1)
    {
        ...
        if(gd_l == 1) //左侧有障碍物情况
        {
            if(current_time - last_time >= 25) //25ms检测一次
            {
                ...
                //停车条件
                if(mecanum_control->stp_distance_l <= 230 && mecanum_control->stp_distance_l >= 180)
                {
                    jg_conter++;
                    if(jg_conter >= 2) //消抖
                    {
                ...
                //右走
                else if(mecanum_control->stp_distance_l < 180)
                {
                    jg_conter++;
                    if(jg_conter >= 2) //消抖
                    {
        ...
        else if(gd_r == 1) //右侧有障碍物情况
        {
            if(current_time - last_time >= 20)
            {
                ...
                //停车条件
                if(mecanum_control->stp_distance_r <= 230 && mecanum_control->stp_distance_r >= 180)
                {
                    jg_conter++;
                    if(jg_conter >= 2)
                    {
                ...
                //左走
                else if(mecanum_control->stp_distance_r < 180) //右侧近，往左走
                {
                    jg_conter++;
                    if(jg_conter >= 2)
                    {
...

{
    ...
    static fp32 vy=0.0f;
    static fp32 act_p=0.2f; //启动加速比例（相对总距离）
    static fp32 stop_p=0.4f; //停止减速比例（相对总距离）
    static fp32 direction_x, direction_y;
    static uint32_t start_time=0;
    fp32 a =speed/1.0f; //加速度 500mm/s²
    fp32 current_speed;
